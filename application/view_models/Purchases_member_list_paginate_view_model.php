...
/**
 * Purchases List Paginate View Model
 *
 ...
 * @author Ryan Wong
 */
class Purchases_member_list_paginate_view_model
{
  protected $_library;
  protected $_base_url      = '';
  protected $_heading       = 'Purchases';
  protected $_total_rows    = 0;
  protected $_format_layout = '';
  protected $_per_page      = 10;
  protected $_page;
  protected $_num_links     = 5;
  protected $_column        = [ 'Action', 'ID', 'School', 'Professor', 'Word Count', 'Year', 'Purchase Date', 'Status'];
  protected $_field_column  = [ '','id', 'inventory_id', 'inventory_id', 'inventory_id', 'inventory_id', 'purchase_user_id', 'status'];
  protected $_list          = [];
  protected $_links         = '';
  ...
  public function to_json()
  {
    ...
    foreach ($list as $key => $value)
    {
      ...
      $clean_list_entry['purchase_user_id'] = $list[$key]->purchase_user_id;
      $clean_list_entry['sale_user_id']     = $list[$key]->sale_user_id;
      $clean_list_entry['inventory_id']     = $list[$key]->inventory_id;
      $clean_list_entry['order_date']       = $list[$key]->order_date;
      $clean_list_entry['order_time']       = $list[$key]->order_time;
      ...
      $clean_list_entry['stripe_intent']    = $list[$key]->stripe_intent;
